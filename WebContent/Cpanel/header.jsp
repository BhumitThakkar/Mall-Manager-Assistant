...
<%System.gc();%>			<!-- Explicit Garbage Collector -->
...
asyncContext.setTimeout(9999999); */
...
String txtMallPin = request.getParameter("txtMallPin");											/* Came pincode after selecting a mall from dropdown on the same page */
String txtMallCategory = request.getParameter("txtMallCategory");							/* Came Category after selecting a mall from dropdown on the same page */
String txtMallCategoryFull = null;
if(txtMallPin!=null && txtMallCategory!=null)
txtMallCategoryFull = mallServe.getMallCategory(txtMallCategory);
String txtMallArea = request.getParameter("txtMallArea");										/* Came area after selecting a mall from dropdown on the same page */
String txtDropdownMallName = request.getParameter("txtDropdownMallName");			/* Came name after selecting a item from dropdown on the same page */
String changeCurrentMall = request.getParameter("changeCurrentMall");					/* Came "yes" after selecting a item from dropdown on the same page to change the current mall*/
List<MallBranchGodown> managerMallList = null;
%>
	<div class="control-group">

		<div class="dropdown">
			<button class="btn btn-secondary dropdown-toggle" type="button" data-toggle="dropdown">Select Mall
				<span class="caret"></span></button>
				<ul class="dropdown-menu" style="margin-top:-10px; max-height: 15em; overflow: auto;">
					<%
					if(manager != null){
						managerMallList = mallServe.selectMallsOfCurrentManager(manager);
						currentMall = (MallBranchGodown) request.getSession().getAttribute("Current Mall");
						if(currentMall == null){
							txtMallPin = managerMallList.get(0).getPincode().toString();
							txtDropdownMallName = managerMallList.get(0).getMall_name();
							txtMallArea = managerMallList.get(0).getArea();
							txtMallCategory = managerMallList.get(0).getM_b_g_mb();
							txtMallCategoryFull = mallServe.getMallCategory(txtMallCategory);
							currentMall = mallServe.getCurrentMall(txtDropdownMallName,txtMallPin,txtMallCategory);
							request.getSession().setAttribute("Current Mall",currentMall);											
						}
						if("yes".equals(changeCurrentMall)){
							currentMall = mallServe.getCurrentMall(txtDropdownMallName,txtMallPin,txtMallCategory);
							request.getSession().setAttribute("Current Mall",currentMall);											
						}
						for (MallBranchGodown ob : managerMallList) {
							String pin = ob.getPincode().toString();
							String mallCategory = ob.getM_b_g_mb();
							String mallName = ob.getMall_name();
							String mallArea = ob.getArea();
							String mallCategoryFull = mallServe.getMallCategory(ob.getM_b_g_mb());
							String toCallMethod = mallName+"("+mallArea+")->"+mallCategoryFull+" with pinCode-> "+pin;
						%>
						<li><a href="?changeCurrentMall=yes&txtDropdownMallName=<%=mallName%>&txtMallPin=<%=pin%>&txtMallCategory=<%=mallCategory%>&txtMallArea=<%=mallArea%>"><%= toCallMethod %> </a></li>
						<% }
						}%>
					</ul>
			<%
			currentMall = (MallBranchGodown) request.getSession().getAttribute("Current Mall");
			txtMallCategoryFull = mallServe.getMallCategory(currentMall.getM_b_g_mb());
			%>
			<input style="margin-top: 10px; width:200px;" type="text" name="txtDropdownMallName" id="txtDropdownMallName" onblur="checkAndSign();" value="<%=currentMall.getMall_name()%>" class="input-xlarge" required disabled/>
			<input style="margin-top: 10px; width:200px;" type="text" name="txtMallArea" id="txtMallArea" value="<%=currentMall.getArea()%>" class="input-xlarge" required disabled/>
			<input style="margin-top: 10px; width:200px;" type="text" name="txtMallCategory" id="txtMallCategory" value="<%=txtMallCategoryFull%>" class="input-xlarge" required disabled/>
			<input style="margin-top: 10px; width:200px;" type="text" name="txtMallPin" id="txtMallPin" onblur="validatePin();" value="<%=currentMall.getPincode()%>" class="input-xlarge" required disabled/>
			
			<a href="add_mall.jsp" class="btn btn-info btn-lg">
	          <span>Add New Mall</span>
	        </a>
			</div>
			...
			if(manager != null){
				currentMall = (MallBranchGodown) request.getSession().getAttribute("Current Mall");
				List<Manager> mallManagerList = managerServe.selectManagersOfCurrentMall(currentMall);
				System.out.println(mallManagerList.size());
				if(mallManagerList.size()==1){
				}
				else{
			%>
			<div>
			<button class="btn btn-secondary" type="button">Manager Partners:-
			</button>
				<%for (Manager ob : mallManagerList) {
					if(ob.getMail_id().equals(manager.getMail_id())){}
					else{
					%>
					<span class="btn btn-info btn-lg"><%= ob.getName() %></span>
			<% }
			}
			%></div>
			<%
		...
		<div class="container-fluid">
			<a id="brand"><span class="btn btn-info btn-lg">Mall Manager Assistant</span></a>
...
<!-- 				<ul class="icon-nav">
					<li class='dropdown language-select'>
						<a href="#" class='dropdown-toggle' data-toggle="dropdown"><img src="img/demo/flags/in.gif" alt=""><span>India</span></a>
						<ul class="dropdown-menu pull-right">
							<li>
								<a href="#"><img src="img/demo/flags/br.gif" alt=""><span>Us</span></a>
							</li>
							<li>
 ...
 -->
 				<div class="dropdown">
					<a href="#" class='dropdown-toggle' data-toggle="dropdown"><span class="glyphicon glyphicon-user"></span> <%=manager.getName()%></a>
					<ul class="dropdown-menu pull-right">
						<li>
							<a href="account.jsp?currentUser=<%=manager.getMail_id()%>">Account settings</a>
						</li>
						<li>
							<a id="logout">Sign out</a>					<!-- href defined in myjquery.js -->
						</li>
					</ul>
				</div>
			</div>
